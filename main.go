...
func clipHandler(w http.ResponseWriter, r *http.Request) {
	...
	cmd := exec.Command("ffmpeg", "-i", filepath.Join("uploads", videoFile), "-ss", start, "-to", end, "-c:v", "copy", "-c:a", "aac", "-af", "loudnorm", "-threads", "0", clipPath)
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func autoClipHandler(w http.ResponseWriter, r *http.Request) {
	...
	marginV := r.FormValue("marginV")

	if start == "" || end == "" || videoFile == "" || alignment == "" || marginV == "" {
		http.Error(w, "Missing required form values: start, end, videoFile, alignment, and marginV", http.StatusBadRequest)
		return
	}
	...
	forceStyle := fmt.Sprintf("Alignment=%s\\,FontName=Arial\\,FontSize=18\\,PrimaryColour=&Hffffff\\,BackColor=&H80000000\\,BorderStyle=1\\,Outline=1\\,Shadow=0\\,MarginV=%s", alignment, marginV)
	vf_string := fmt.Sprintf("crop=in_h*9/16:in_h,scale=1080:1920,setsar=1,subtitles=%s:force_style='%s'", strings.ReplaceAll(subtitlePath, "\\", "/"), forceStyle)
	cmd := exec.Command("ffmpeg", "-y", "-ss", start, "-to", end, "-i", filepath.Join("uploads", videoFile), "-vf", vf_string, "-c:a", "aac", "-af", "loudnorm", "-threads", "0", clipPath)
	output, err = cmd.CombinedOutput()
	if err != nil {
		log.Printf("ffmpeg error: %s\n%s", err, output)
		http.Error(w, fmt.Sprintf("Failed to create clip: %s", output), http.StatusInternalServerError)
